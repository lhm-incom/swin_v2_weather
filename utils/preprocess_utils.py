    ...
    def forward(self, data):
        ...
        if self.params.add_zenith:
            return (
                ...
                tar,
                tzen,
            )  # map(lambda x: x.to(self.static_features.device, dtype = torch.float), [inp, tar, tzen])
        else:
            return (
                inp,
                tar,
                None,
            )  # list(map(lambda x: x.to(self.device, dtype = torch.float), [inp, tar])) + [None]
