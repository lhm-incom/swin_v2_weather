...
class ERA5ES(object):
    ...
    def _get_files_stats(self, enable_logging):
        ...
        )  # set to total; in modulus this can be smaller but not used currently
        ...
        if self.add_zenith:
            self.zen_inp_buffs = [
                cpx.zeros_pinned(
                    (1, self.img_shape_x, self.img_shape_y), dtype=np.float32
                ),
                cpx.zeros_pinned(
                    (1, self.img_shape_x, self.img_shape_y), dtype=np.float32
                ),
    ...
    def _compute_zenith_angle(self, local_idx, year_idx, time_step_hours=6):
        ...
        def calculate_cos_zenith(start_idx: int, end_idx: int) -> np.ndarray:
            ...
            return np.stack(
                cos_zenith, axis=0
            )  # Stack the angles into a multi-dimensional array
        ...
        cos_zenith_tar = calculate_cos_zenith(
            local_idx + self.dt, local_idx + self.dt * (self.n_future + 1) + 1
        )
    ...
    def __call__(self, sample_info):
        ...
        if self.add_zenith:
            tmp_zen_inp, tmp_zen_tar = self._compute_zenith_angle(
                local_idx, year_idx
            )  # compute the zenith angles for the input.
            zen_inp[...] = tmp_zen_inp[
                ..., : self.img_shape_x, : self.img_shape_y
            ]  # adjust to match input dimensions
            zen_tar[...] = tmp_zen_tar[
                ..., : self.img_shape_x, : self.img_shape_y
            ]  # adjust to match input dimensions
