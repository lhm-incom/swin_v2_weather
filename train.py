...
class Trainer:
    ...
    def __init__(self, params, args):
        ...
        self.ckpt_fn = (
            ...
            if hasattr(params, "activation_ckpt") and params.activation_ckpt
            else ckpt_identity
        )
    ...
    def build_and_launch(self):
        ...
        if self.sweep_id:
            ...
            exp_dir = os.path.join(
                *[self.root_dir, "sweeps", self.sweep_id, self.config, jid]
            )
        else:
        ...
        self.params["checkpoint_path"] = os.path.join(
            exp_dir, "training_checkpoints/ckpt.tar"
        )
        self.params["best_checkpoint_path"] = os.path.join(
        ...
        if self.world_rank == 0:
            logging_utils.log_to_file(
                logger_name=None, log_filename=os.path.join(exp_dir, "out.log")
            )
        ...
        self.valid_data_loader, self.valid_dataset = get_data_loader_distributed(
            self.params, self.params.valid_data_path, dist.is_initialized(), train=False
        )
        ...
        if self.world_rank == 0:
            ...
            for key, value in self.params.params.items():
                ...
                with open(
                    os.path.join(self.params["experiment_dir"], "hyperparams.yaml"), "w"
                ) as hpfile:
        ...
        if self.params.optimizer_type == "adam":
            ...
            self.optimizer = optimizers.FusedLAMB(
                self.model.parameters(), lr=self.params.lr, max_grad_norm=5.0
            )
        else:
            raise Exception(
                f"optimizer type {self.params.optimizer_type} not implemented"
            )
        ...
        if self.params.finetune and not self.params.resuming:
            ...
            if self.log_to_screen:
                logging.info(
                    "Loading checkpoint %s" % self.params.pretrained_checkpoint_path
                )
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if self.log_to_screen:
                ...
                logging.info(
                    "Training time = {}, Valid time = {}".format(tr_time, valid_time)
                )
                logging.info(
    ...
    def validate_one_epoch(self):
        ...
        if hasattr(self.params, "track_channels"):
            idxes = [
                self.params.channel_names.index(varname)
                for varname in self.params.track_channels
            ]
